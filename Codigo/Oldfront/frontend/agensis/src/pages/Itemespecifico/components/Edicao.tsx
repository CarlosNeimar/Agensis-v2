...
export const Edicao: React.FC<EdicaoProps> = ({ item, onItemChange, onSave }) => {
  ...
  const handleIniciarProjeto = async () => {
    ...
    try {
      ...
      console.log('Resposta da API (Iniciar Projeto):', projetoResponse.data);

      // Create a transaction for the project
      // // const novaTransacao = {
      // //   nome: `${item.proposta?.nome || ''}${formatDate(item.proposta?.prazo ?? null)}`,
      // //   descricao: `Transação inicial para o projeto ${item.id}`,
      // //   tipo: 'entrada', // Assuming starting a project is an expense
      // //   valor: item.proposta?.valor || 0, // Use the project value or 0 if not available
      // //   data: format(new Date(), 'yyyy-MM-dd'), // Today's date
      // //   recorrente: false,
      // //   status: 'pendente' // You might want to change this based on your business logic
      // // };

      // // const token = sessionStorage.getItem("authorization");
      // // const transacaoResponse = await axios.post("http://localhost:3002/api/transacoes", novaTransacao, {
      // //   headers: {
      // //     "Content-Type": "application/json",
      ...
      // console.log('Resposta da API (Criar Transação):', transacaoResponse.data);

      alert("Projeto iniciado e transação criada com sucesso!");
    } catch (error) {
      console.error("Erro ao iniciar projeto ou criar transação:", error);
